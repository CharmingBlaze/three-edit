/**
 * @fileoverview Plane primitive creation function
 * Creates a plane with specified dimensions and returns an EditableMesh
 */
 ...
 * @param {number} params.height - Height (Z dimension), defaults to 1
 ...
 * @param {string} params.name - Mesh name, defaults to 'Plane'
 * @returns {EditableMesh} Plane mesh
 */
export function createPlane(params = {}) {
  ...
  const halfHeight = (height * scaleZ) / 2;
  ...
  for (let z = 0; z <= heightSegments; z++) {
    for (let x = 0; x <= widthSegments; x++) {
      const pos = transformVertex(
        ...
        0, // Y is always 0 for a flat plane
        (z / heightSegments - 0.5) * height
      );
      
      const vertex = new Vertex(pos.x, pos.y, pos.z);
      vertex.setNormal(0, 1, 0); // Upward normal for plane
      vertices.push(vertex);
      mesh.addVertex(vertex);

      // Add UV coordinates
      const uv = new UV(x / widthSegments, z / heightSegments, vertex.id);
      mesh.addUV(uv);
    }
  }
  ...
  for (let z = 0; z < heightSegments; z++) {
    for (let x = 0; x < widthSegments; x++) {
      ...
      const b = z * (widthSegments + 1) + x + 1;
      const c = (z + 1) * (widthSegments + 1) + x + 1;
      const d = (z + 1) * (widthSegments + 1) + x;
  ...
  if (!validation.isValid) {
    console.warn('Plane validation issues:', validation.errors);
    mesh.fixIssues();
  }
