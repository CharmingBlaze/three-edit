/**
 * @fileoverview Cone primitive creation function
 * Creates a cone with specified radius, height and segments and returns an EditableMesh
 */
 ...
 * @param {number} params.radius - Base radius, defaults to 0.5
 ...
 * @param {boolean} params.openEnded - Whether to create base cap, defaults to false
 ...
 * @param {string} params.name - Mesh name, defaults to 'Cone'
 * @returns {EditableMesh} Cone mesh
 */
export function createCone(params = {}) {
  ...
  for (let y = 0; y <= heightSegments; y++) {
    ...
    const currentRadius = radius * (1 - v); // Radius decreases from base to tip
    ...
    for (let x = 0; x <= radialSegments; x++) {
      ...
      // Calculate normal for side faces (pointing outward from cone)
      const normalX = Math.cos(theta);
      const normalZ = Math.sin(theta);
      const normalY = radius / height; // Slope of cone
      const normalLength = Math.sqrt(normalX * normalX + normalY * normalY + normalZ * normalZ);
      vertex.setNormal(normalX / normalLength, normalY / normalLength, normalZ / normalLength);
  ...
  if (!openEnded && radius > 0) {
    ...
    const baseCenter = transformVertex(0, -halfHeight, 0);
    const baseCenterVertex = new Vertex(baseCenter.x, baseCenter.y, baseCenter.z);
    baseCenterVertex.setNormal(0, -1, 0);
    vertices.push(baseCenterVertex);
    mesh.addVertex(baseCenterVertex);

    const baseUV = new UV(0.5, 0.5, baseCenterVertex.id);
    mesh.addUV(baseUV);
  ...
  if (!validation.isValid) {
    console.warn('Cone validation issues:', validation.errors);
    mesh.fixIssues();
  }
