...
const { toBufferGeometry } = require('./src/conversion');

console.log('🎯 Three-Edit Basic Geometry Test\n');

// Test function
function testPrimitive(name, createFunction, options = {}) {
    ...
    try {
        ...
        const hasUVs = mesh.vertices.some(v => v.uv);
        const hasNormals = mesh.faces.some(f => f.normal);
...
if (successfulTests.length > 0) {
    ...
    const avgCreationTime = successfulTests.reduce((sum, r) => sum + r.creationTime, 0) / successfulTests.length;
